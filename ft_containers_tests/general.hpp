...
#include <unistd.h>
#include <iostream>
#include "../Algorithm.hpp" // your utilities
...
void vector_tests() {
	...
	printElement("constructors");(constructor_test(vector) == constructor_test(my_vector)) ? printElement("OK") : printElement("FAILED");t1 = g_end1 - g_start1, t2 = g_end2 - g_start2;(t1 >= t2) ? printElement(GREEN + std::to_string(t2) + "ms" + RESET) : printElement(RED + std::to_string(t2) + "ms" + RESET);(t1 > t2) ? printElement(RED + std::to_string(t1) + "ms" + RESET) : printElement(GREEN + std::to_string(t1) + "ms" + RESET);cout << endl;
